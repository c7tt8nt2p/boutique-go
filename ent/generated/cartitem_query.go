...
func (ciq *CartItemQuery) QueryCartIDOwner() *CartQuery {
...
// QueryProductIDOwner chains the current query on the "product_id_owner" edge.
func (ciq *CartItemQuery) QueryProductIDOwner() *ProductQuery {
...
// the "cart_id_owner" edge. The optional arguments are used to configure the query builder of the edge.
func (ciq *CartItemQuery) WithCartIDOwner(opts ...func(*CartQuery)) *CartItemQuery {
	query := (&CartClient{config: ciq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ciq.withCartIDOwner = query
	return ciq
}

// WithProductIDOwner tells the query-builder to eager-load the nodes that are connected to
// the "product_id_owner" edge. The optional arguments are used to configure the query builder of the edge.
func (ciq *CartItemQuery) WithProductIDOwner(opts ...func(*ProductQuery)) *CartItemQuery {
	query := (&ProductClient{config: ciq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ciq.withProductIDOwner = query
	return ciq
}
...
func (ciq *CartItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*CartItem, error) {
	...
	if query := ciq.withProductIDOwner; query != nil {
		if err := ciq.loadProductIDOwner(ctx, query, nodes, nil,
			func(n *CartItem, e *Product) { n.Edges.ProductIDOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ciq *CartItemQuery) loadProductIDOwner(ctx context.Context, query *ProductQuery, nodes []*CartItem, init func(*CartItem), assign func(*CartItem, *Product)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*CartItem)
	for i := range nodes {
...
func (ciq *CartItemQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := ciq.ctx.Fields; len(fields) > 0 {
		...
		if ciq.withCartIDOwner != nil {
			_spec.Node.AddColumnOnce(cartitem.FieldCartID)
		}
		if ciq.withProductIDOwner != nil {
