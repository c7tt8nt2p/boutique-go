...
func (aq *AuthQuery) QueryUserIDOwner() *UserQuery {
...
func (aq *AuthQuery) Clone() *AuthQuery {
	...
	return &AuthQuery{
		...
		withUserIDOwner: aq.withUserIDOwner.Clone(),
...
func (aq *AuthQuery) WithUserIDOwner(opts ...func(*UserQuery)) *AuthQuery {
	query := (&UserClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withUserIDOwner = query
	return aq
}
...
func (aq *AuthQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Auth, error) {
	...
	if query := aq.withUserIDOwner; query != nil {
		if err := aq.loadUserIDOwner(ctx, query, nodes, nil,
			func(n *Auth, e *User) { n.Edges.UserIDOwner = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthQuery) loadUserIDOwner(ctx context.Context, query *UserQuery, nodes []*Auth, init func(*Auth), assign func(*Auth, *User)) error {
...
func (aq *AuthQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withUserIDOwner != nil {
			_spec.Node.AddColumnOnce(auth.FieldUserID)
		}
	}
