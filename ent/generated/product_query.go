...
func (pq *ProductQuery) QueryCartItem() *CartItemQuery {
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withCartItem: pq.withCartItem.Clone(),
...
func (pq *ProductQuery) WithCartItem(opts ...func(*CartItemQuery)) *ProductQuery {
	query := (&CartItemClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withCartItem = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withCartItem; query != nil {
		if err := pq.loadCartItem(ctx, query, nodes,
			func(n *Product) { n.Edges.CartItem = []*CartItem{} },
			func(n *Product, e *CartItem) { n.Edges.CartItem = append(n.Edges.CartItem, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadCartItem(ctx context.Context, query *CartItemQuery, nodes []*Product, init func(*Product), assign func(*Product, *CartItem)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(cartitem.FieldProductID)
	}
	query.Where(predicate.CartItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.CartItemColumn), fks...))
	}))
