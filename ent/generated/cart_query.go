...
func (cq *CartQuery) QueryUserIDOwner() *UserQuery {
...
// QueryCartItem chains the current query on the "cart_item" edge.
func (cq *CartQuery) QueryCartItem() *CartItemQuery {
...
func (cq *CartQuery) Clone() *CartQuery {
	...
	return &CartQuery{
		...
		withCartItem:    cq.withCartItem.Clone(),
...
func (cq *CartQuery) WithUserIDOwner(opts ...func(*UserQuery)) *CartQuery {
	query := (&UserClient{config: cq.config}).Query()
...
// WithCartItem tells the query-builder to eager-load the nodes that are connected to
// the "cart_item" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CartQuery) WithCartItem(opts ...func(*CartItemQuery)) *CartQuery {
	query := (&CartItemClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCartItem = query
	return cq
}
...
func (cq *CartQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Cart, error) {
	...
	if query := cq.withUserIDOwner; query != nil {
		if err := cq.loadUserIDOwner(ctx, query, nodes, nil,
			func(n *Cart, e *User) { n.Edges.UserIDOwner = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCartItem; query != nil {
		if err := cq.loadCartItem(ctx, query, nodes,
			func(n *Cart) { n.Edges.CartItem = []*CartItem{} },
			func(n *Cart, e *CartItem) { n.Edges.CartItem = append(n.Edges.CartItem, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CartQuery) loadUserIDOwner(ctx context.Context, query *UserQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *User)) error {
...
func (cq *CartQuery) loadCartItem(ctx context.Context, query *CartItemQuery, nodes []*Cart, init func(*Cart), assign func(*Cart, *CartItem)) error {
	...
	query.Where(predicate.CartItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(cart.CartItemColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
